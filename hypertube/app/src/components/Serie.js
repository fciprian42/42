...
class Series extends Component
{
    constructor(props)
    {
        ...
        this.state = {liked: false, hash: 0, likes: 0, quality: 720, toggle: false, commented: false, play: false};
    }

    submit = values => {
        const { dispatch, match } = this.props;

        if (dispatch && values && match)
        {
            this.setState({commented: true});
            dispatch(usersActions.postComment(values.message, match.params.id, true));
        }
    };
    ...
    componentDidMount()
    {
        ...
        else
        {
            ...
            {
                if (match && match.params.id && users && users.session)
                {
                    axios.get(`http://localhost:8080/api/series/${match.params.id}`, {headers: {authorization: user && user.token}})
                        .then(response => {
                            if (response && response.data && response.data.serie)
                            {
                                this.setState({imdb: response.data.serie.id, likes: response.data.serie.likes.length, data: response.data.serie, liked: response.data.serie.liked});
                                dispatch({type: moviesConstants.GET_COMMENTS, comments: response.data.serie.comments, commented: false});
                                document.title = "Hypertube - " + (response.data.serie && response.data.serie.data.Title);
                                this.getAvatar();
                                this.hasComment();
                            }
                            else
    ...
    deleteComment = () => {
        ...
        if (dispatch && match)
            dispatch(usersActions.deleteComment(match.params.id, true));
    };
    ...
    addToBookmarks = () => {
        ...
        if (dispatch)
            dispatch(usersActions.addToBookmarks(match.params.id, true));
    };
    ...
    likeMovie = () => {
        ...
        if (!this.state.liked)
        {
            ...
            if (dispatch)
            {
                dispatch(moviesActions.like(match.params.id, true));
        ...
        else
        {
            ...
            if (dispatch)
            {
                dispatch(moviesActions.unlike(match.params.id, true));
                this.setState({likes: this.state.likes - 1});
            }
        }
    };
    ...
    launchPlayer = () => {
        ...
        if (this.state.data)
        {
            hash = this.state.data.torrents ? parseTorrent(this.state.data.torrents[0].magnetUrl).infoHash : null;
            this.setState({play: true, hash: hash});
        }
    };

    render()
    {
        const { location, users, alerts, currentLanguage , movies, match} = this.props;
        ...
        const playButton = (
            <div className="btn-play">
                <img  src={require('../style/images/play_btn.png')} onClick={this.launchPlayer} alt="play"/>
            </div>
        );
        ...
        const playerVideo = (
            <video controlsList="nodownload" id="video" src={`http://localhost:8080/api/stream?hash=${hash && hash}&id=${match && match.params && match.params.id}&token=${users.session && me.token}&controlsList="nodownload"&stop=false`} width="800" controls crossOrigin="anonymous" style={{position: "relative", height: "100%", backgroundColor: "black", width: "100%"}}>
                    ...
                    <div className="global-c">
                        ...
                        <section className="content-box">
                            ...
                            <div className="video" id="video-player">
                                {!this.state.play && hash === 0 ? playButton : playerVideo}
                            </div>
                                    ...
                                    {movies && movies.comments.length > 0 ? movies.comments.map((value, key) => {
                                        return (
                                            <div className={`comment-section ${this.state.commented ? 'commented-s' : 'no-commented-s'}`} key={key}>
                                                <div>
                                                    <p className="comment-author"><Link to={`/users/${value.username}`}>{value.username}</Link>{value.username === me.username ? <i onClick={this.deleteComment} style={{marginLeft: 10, fontSize: 14, color: "#ff4d4d", cursor: "pointer"}} className="fas fa-times"></i> : null}</p>
                            ...
                            <div className="options">
                                ...
                                <div className="movie-quality-container">
                                    {hash && hash === null ? NoTorrents : null}
                                </div>
                            </div>
                        </section>
                    </div>
